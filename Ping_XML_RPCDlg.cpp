...
CPing_XML_RPCDlg* pDlg;

// 每个线程中对应字符串的数量
UINT g_nThreadSTRNumber = 0;


CPing_XML_RPCDlg::CPing_XML_RPCDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CPing_XML_RPCDlg::IDD, pParent)
	, m_nThreadNumber(0)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CPing_XML_RPCDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPing_XML_RPCDlg, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_BEGIN, &CPing_XML_RPCDlg::OnBnClickedBegin)
	ON_BN_CLICKED(IDC_END, &CPing_XML_RPCDlg::OnBnClickedEnd)
	ON_BN_CLICKED(IDC_OPENFILE, &CPing_XML_RPCDlg::OnBnClickedOpenfile)
...
// CPing_XML_RPCDlg 消息处理程序

BOOL CPing_XML_RPCDlg::OnInitDialog()
{
...
void CPing_XML_RPCDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR CPing_XML_RPCDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}


void CPing_XML_RPCDlg::OnBnClickedBegin()
{
	// 判断是否可以点击开始按钮
	if( IsBegin() == FALSE )
		return ;// 不可以点击
...
void CPing_XML_RPCDlg::OnBnClickedEnd()
{
	// 禁止按钮不可以点击了
	GetDlgItem(IDC_END)->EnableWindow(FALSE);
...
// 控制状态
void CPing_XML_RPCDlg::ControlStateControl( BOOL nBool )
{
...
void CPing_XML_RPCDlg::OnBnClickedOpenfile()
{
...
BOOL CPing_XML_RPCDlg::IsBegin( VOID )
{
	...
	{}else{
		AfxMessageBox(_T("您未选择需要Ping的搜索引擎"));
		return FALSE;
	}		
	...
	{ // 未选择
		AfxMessageBox(_T("您未选择需要Ping的URL文件"));
		return FALSE;
	}
...
DWORD WINAPI InitThreadProc(LPVOID pParam)
{	
	CPing_XML_RPCDlg* pDlg = (CPing_XML_RPCDlg*)pParam;
...
// 初始化URL数量
LRESULT CPing_XML_RPCDlg::InitListURLNumber( WPARAM wParam, LPARAM lParam )
{
	CString strText;
...
// 修改URL数量
LRESULT CPing_XML_RPCDlg::ChangeListURLNumber( WPARAM wParam, LPARAM lParam )
{
...
LRESULT CPing_XML_RPCDlg::ChangeListURLNumberThreadError(WPARAM wParam, LPARAM lParam)// 整个线程失败
{
...
LRESULT CPing_XML_RPCDlg::ChangStat(WPARAM wParam, LPARAM lParam)
{
	GetDlgItem(IDC_END)->EnableWindow(FALSE);
...
// 点击Check 按钮
void CPing_XML_RPCDlg::OnCheckButton( UINT nID )
{
	// 发送消息, 取消选中
	if( nID != IDC_CHECK1 )
	{
...
void CPing_XML_RPCDlg::OnTimer(UINT_PTR nIDEvent)
{
	if ( nIDEvent == 1000 )
	{
		for ( int i = 0 ; i<5; ++i )
...
void SearchEnginesBaidu(char* pURL, CURL* curl )
{
    ...
    // 设置URL
    curl_easy_setopt( curl, CURLOPT_URL,  "http://ping.baidu.com/ping/RPC2" );  
    //设置返回html源码的方式
    curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, write_data_baidu);
    // 构造内容
    char szPostData[1024] = {0};
    sprintf_s(szPostData,1024,"<?xml version=\"1.0\"?> <methodCall> <methodName>weblogUpdates.ping</methodName> <params> <param> <value><string>%s</string></value> </param><param><value><string>%s</string></value> </param> </params> </methodCall>",pURL,pURL);
    // 指定post内容
    ...
    // 判断在循环外面是否为 CURLE_OK
    if (res!=CURLE_OK)
        pDlg->SendMessage((WM_USER+2),0,1); // 发送失败
...
LRESULT CPing_XML_RPCDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
...
void CPing_XML_RPCDlg::OnDestroy()
{
    CDialog::OnDestroy();
...
INT_PTR CPing_XML_RPCDlg::DoModal()
{
    // 初始化信息
...
void CPing_XML_RPCDlg::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    
    CDialog::OnKeyDown(nChar, nRepCnt, nFlags);
}

BOOL CPing_XML_RPCDlg::PreTranslateMessage(MSG* pMsg)
{
...
void CPing_XML_RPCDlg::WinHelp(DWORD dwData, UINT nCmd)
{
